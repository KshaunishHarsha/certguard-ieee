...
import { getPreviewHash } from '@/ai/flows/get-preview-hash';
...
export function IssueCertificateForm() {
  ...
  // It now calls a server action to compute the hash.
  useEffect(() => {
    const updatePreview = async () => {
      ...
      if (id && studentName && course && issuingInstitution && grade && rollNumber && year) {
        try {
          ...
          console.error("Failed to get preview hash from server:", error);
          setPreviewQrValue('');
        }
      } else {
        setPreviewQrValue('');
    ...
    // Debounce the call to avoid too many server requests
        ...
        <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="institution" placeholder="e.g., NMIMS" value={details.issuingInstitution || ''} onChange={e => setDetails({...details, issuingInstitution: e.target.value})} />
              </div>
