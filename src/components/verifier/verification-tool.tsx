...
export function VerificationTool() {
  ...
  const handleVerification = async (
    ...
    setResult(null);
    try {
      const response = await verifyCertificateAuthenticity({
        qrDataUri,
        documentDataUri,
      });
      setResult(response);
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      if (!imageData)
        throw new Error("Could not process the file to get image data.");
      ...
      toast({
        ...
        description:
          error.message || "Could not find a QR code in the uploaded file.",
        variant: "destructive",
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
          ...
          <div className="p-6 border-b md:border-b-0 md:border-r">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Use your device's camera to scan a physical certificate.
            </p>
              ...
              {hasCameraPermission === false && (
                <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-white p-4">
                  ...
                  <p className="text-center font-semibold">
                    Camera access is required for live QR code scanning.
                  </p>
                </div>
              )}
          ...
          <div className="p-6">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Verify a digital certificate by uploading its file.
            </p>
            <div
              className={`flex flex-col items-center justify-center w-full aspect-video border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted transition-colors ${
                isDragging ? "bg-muted border-primary" : "bg-transparent"
              }`}
